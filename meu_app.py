...
if len(st.session_state["carteira"]) > 0:
    ativos_unicos = list(set([a["Ticker"] for a in st.session_state["carteira"]]))
    for ativo in ativos_unicos[:10]: # Limita a 10 para o letreiro não ficar infinito
...
# ==========================================
# --- 5. GRÁFICO E RANKING (LADO A LADO SEGURO) ---
# ==========================================
st.subheader("🌐 Panorama do Mercado e Seus Ativos")

opcoes_grafico = {"Mercado Geral (Ibovespa)": "BMFBOVESPA:IBOV"}
ativos_graf = list(set([ativo["Ticker"] for ativo in st.session_state["carteira"]]))
...
with col_ranking:
    ...
    with c1:
        ...
        for item in altas:
            st.markdown(f"**{item['Ativo']}**: R$ {item['Preço']:.2f} (<span style='color:#00e676'>+{item['Var%']:.2f}%</span>)", unsafe_allow_html=True)
            
    with c2:
        st.markdown("<h4 style='color: #ff4b4b; margin-bottom: 5px;'>⬇️ Maiores Baixas</h4>", unsafe_allow_html=True)
        for item in baixas:
            st.markdown(f"**{item['Ativo']}**: R$ {item['Preço']:.2f} (<span style='color:#ff4b4b'>{item['Var%']:.2f}%</span>)", unsafe_allow_html=True)

st.divider()

# ==========================================
# --- 6. TABELA ESTILO INVESTING.COM ---
# ==========================================
if len(st.session_state["carteira"]) > 0:
        ...
        for idx, row in df_agrupado.iterrows():
            ...
            if info:
                ...
                tabelas.append({
                    ...
                    "Qtd": int(qnt), "Preço Médio": preco_medio, "Lucro/Prej.": lucro
                })
    ...
    if tabelas:
        ...
        styled = df_view.style.format({
            ...
            "Variação": "{:+.2f}", "Var%": "{:+.2%}", "Preço Médio": "R$ {:.2f}", "Lucro/Prej.": "R$ {:+.2f}"
        ...
        col3.metric("Rentabilidade Geral", f"{rent:.2f}%")
else:
